...
public abstract class AbstractInstanceRegistry implements InstanceRegistry {
    ...
    /**
     * 该对象就是存储应用实例的容器 可以看到基于AP的实现并没有为数据作持久化
     * key1： 代表 appName    key2： 对应某个instance的id
     */
    private final ConcurrentHashMap<String, Map<String, Lease<InstanceInfo>>> registry
            = new ConcurrentHashMap<String, Map<String, Lease<InstanceInfo>>>();

    /**
     * 某个region 以及对应的client （该client包含了该region的注册中心地址 会定期拉取数据）
     * 但是不像 DiscoveryClient 要将自身注册到远端
     * region 可以理解为集群 也就是在eureka-server的架构中 如果同集群下获取不到应用 是可以尝试从其他集群中获取的
     * 同样虽然是基于AP的实现 但是跨集群是不需要同步的  只要在同一region 下所有的node中进行同步就可
     */
    ...
    // CircularQueues here for debugging/statistics purposes only  记录最近操作过的数据
    // CircularQueues 一开始就要设置大小 并且不支持扩容  同时当队列满时 新元素会替换掉旧元素
    ...
    /**
     * 有点类似窗口的概念 数据按照窗口来划分  可以展示最近一个窗口的调用次数
     */
    ...
    protected void initRemoteRegionRegistry() throws MalformedURLException {
        // 返回其他集群内注册中心地址   实际上默认情况下 eureka是不会设置remoteRegion的节点的
        // key 代表region  value 代表 url(可能是多个)
        ...
        if (!remoteRegionUrlsWithName.isEmpty()) {
            ...
            // url 可以是多个地址拼接   就看做一个地址吧 也就是某个集群下如果找不到应用实例的话 允许从其他集群查找 不确保正确 只是提供了这个入口
    ...
    // for server info use 生成一个实例状态修改的快照
    @Override
     ...
     * @param leaseDuration 指定该实例的续约时间
     * @param isReplication 代表本次执行是否是从其他节点拷贝过来的
     *
     * 将某个实例信息注册到当前节点上
     */
    public void register(InstanceInfo registrant, int leaseDuration, boolean isReplication) {
        try {
            ...
            // 代表尝试注册的实例实际上已经存在了 那么就认为这是一种续约操作
            ...
            // 如果存在旧的实例 则延续之前的 服务启动时间
            if (existingLease != null) {
                lease.setServiceUpTimestamp(existingLease.getServiceUpTimestamp());
            }
            gMap.put(registrant.getId(), lease);
            // 同时将该实例信息添加到 近期注册队列中  这样当获取增量数据时 就可以直接使用这份数据
            recentRegisteredQueue.add(new Pair<Long, String>(
                    System.currentTimeMillis(),
                    registrant.getAppName() + "(" + registrant.getId() + ")"));
            // This is where the initial state transfer of overridden status happens
            // 代表本次实例携带了一个 重置的状态  (非UNKNOWN 代表该状态有效)
            ...
            // Set the status based on the overridden status rules   通过rule对status进行改动
            InstanceStatus overriddenInstanceStatus = getOverriddenInstanceStatus(registrant, existingLease, isReplication);
            // 覆盖status  (status与overriddenStatus 是不同的   overriddenStatus 需要经过rule处理后才可以使用)
    ...
    protected boolean internalCancel(String appName, String id, boolean isReplication) {
        try {
            ...
            // 往队列中追加相关信息  还有将该实例相关的overriddenStatus 移除掉
            ...
            if (leaseToCancel == null) {
                ...
                // 找到实例并触发cancel 方法  就是设置 evict时间戳
                ...
                // 清除缓存   register某个instance时 会清除缓存 而在cancel时 也会清除一次
    ...
    public boolean renew(String appName, String id, boolean isReplication) {
        ...
        if (leaseToRenew == null) {
            ...
            if (instanceInfo != null) {
                ...
                // 使用overriddenRule 处理该续约对象 并返回一个status
    ...
    public boolean statusUpdate(String appName, String id,
        ...
        try {
            ...
            if (lease == null) {
                ...
                // 在更新的同时默认就代表着续约   nacos 上注册的实例是没有续约的概念的 只会有一个UP/DOWN 的状态 必须手动进行注销
                lease.renew();
                ...
                if ((info != null) && !(info.getStatus().equals(newStatus))) {
                    ...
                    // it to the replica's.   如果本次传入的 dirtyTimestamp 更新 那么就更新偏移量
     ...
     */
    @Override
    public boolean deleteStatusOverride(String appName,
                                        String id,
        ...
        try {
            ...
            // 当实例信息不存在时 不需要操作 直接返回
            if (lease == null) {
                return false;
            } else {
                // 当对某个之前存在的实例进行操作时 都会默认进行一次续约
                lease.renew();
                InstanceInfo info = lease.getHolder();
    ...
    /**
     * 当调用cancel时  实例本身同容器中移除 同时设置了一个 evict时间戳  该方法会针对该时间戳做一些事情
     * @param additionalLeaseMs   代表一个补偿时间  该方法本身可以由一个定时器去触发
     */
    public void evict(long additionalLeaseMs) {
        ...
        List<Lease<InstanceInfo>> expiredLeases = new ArrayList<>();   // 记录所有已经超时的实例
        for (Entry<String, Map<String, Lease<InstanceInfo>>> groupEntry : registry.entrySet()) {
            Map<String, Lease<InstanceInfo>> leaseMap = groupEntry.getValue();
            if (leaseMap != null) {
                for (Entry<String, Lease<InstanceInfo>> leaseEntry : leaseMap.entrySet()) {
                    Lease<InstanceInfo> lease = leaseEntry.getValue();
                    // 判断某实例是否超时了  会自动扫描未续约的实例  注册，更新等操作都会自动为实例续约一次  而关闭会直接设置evict时间戳 代表已经超时
        ...
        if (toEvict > 0) {
            ...
            for (int i = 0; i < toEvict; i++) {
                ...
                // 关闭本实例  nacos 对于未进行续约的实例不会自动剔除  (internalCancel的逻辑就是清除缓存 且增加一个 DELETED item 到队列中)
    ...
    public Application getApplication(String appName) {
        // 代表当本地无法找到 该app对应的实例时 是否需要到其他节点查找   默认为false  (也就代表允许从其他region拉取应用)
    ...
    public Applications getApplications() {
        ...
        if (disableTransparentFallback) {
            ...
            // 全局范围的实例  (配置中心配置的该集群下所有 eureka-server上记录的数据)  集群中每个节点的数据最终会一致吗
     ...
     * 访问指定region 下 registry上注册的实例信息  如果参数为空 代表只从本地查询
     */
    public Applications getApplicationsFromMultipleRegions(String[] remoteRegions) {
        ...
        if (includeRemoteRegion) {
            // 遍历目标地区的注册中心 然后通过client拉取数据并追加到容器中
    ...
    /**
     * 代表目标region 是否支持拉取某app    主要是黑白名单级别做限制
     * @param appName
     * @param remoteRegion
     * @return
     */
    ...
    public Applications getApplicationDeltasFromMultipleRegions(String[] remoteRegions) {
        ...
        try {
            write.lock();
            // 难怪要额外维护一个变化的队列    就是为了能快捷定位到发生变动的实例
            Iterator<RecentlyChangedItem> iter = this.recentlyChangedQueue.iterator();
            logger.debug("The number of elements in the delta queue is :{}", this.recentlyChangedQueue.size());
            while (iter.hasNext()) {
                // 注意recentlyChangedQueue内的元素 实际上指向当前注册中心容器中的实例  也就是能确保通过队列获取到的实例就是一个最新状态
                Lease<InstanceInfo> lease = iter.next().getLeaseInfo();
                InstanceInfo instanceInfo = lease.getHolder();
            ...
            // 注意这里 获取增量数据的时候 计算的hashcode 还是全量数据 也就是额外拉取了一次全量数据
    ...
    public InstanceInfo getInstanceByAppAndId(String appName, String id, boolean includeRemoteRegions) {
        ...
        // 实例不为空 并且实例未过期 或者 未开启过期功能  (也就是某些紧急情况 及时实例过期也允许使用)
        if (lease != null
                && (!isLeaseExpirationEnabled() || !lease.isExpired())) {
            // 这里是重置lease的信息后返回
            return decorateInstanceInfo(lease);
        // 代表本registry没有找到目标实例 那么尝试从remoteRegion查询数据
        } else if (includeRemoteRegions) {
            for (RemoteRegionRegistry remoteRegistry : this.regionNameVSRemoteRegistry.values()) {
                // 代表某个远端 registry 下注册了能够提供某APP的实例
                Application application = remoteRegistry.getApplication(appName);
                if (application != null) {
    ...
    private static final class RecentlyChangedItem {
        private long lastUpdateTime; // 变动时间
        private Lease<InstanceInfo> leaseInfo; // 该实例对应的续约信息
    ...
    /* visible for testing */ class EvictionTask extends TimerTask {
        ...
        long getCompensationTimeMs() {
            ...
            // 计算一个补偿时间  如果时间差 小于检测过期的时间间隔 那么就不需要补偿
    ...
    /**
     * 该队列中的item 代表着某个实例发生过变化 当尝试获取增量数据的时候就是查看该队列中有多少数据 可是增量数据又是有一个相对时间的
     * 否则每次拉取到的增量数据可能重复  (上次已经认为它是一个增量数据 下次访问时 得到了相同的增量数据)  所有要做过期清理机制
     * @return
     */
